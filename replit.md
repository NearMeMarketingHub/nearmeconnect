...
- **Admin Dashboard**: Simplified to 3 rows: (1) Active Retainers & Credit Health — per-company tier, monthly credits, used this month, remaining, and status (healthy/at risk/over budget/paused) plus agency totals (`GET /api/admin/credit-health`); (2) My Work — the logged-in admin's active tasks grouped Overdue/Due Today/Due This Week/Next Week, each row opening the TaskDetailPanel in place (`GET /api/admin/my-work`, excludes internal tasks); (3) Agency Operations & SOPs — tasks flagged `isInternal` with assignee name and linked resource button (`GET /api/admin/internal-tasks`).
...
- **Task Workflow**: Supports multi-stage task approval processes, review workflows, and self-service completion for clients. Auto-generation of tasks from approved campaigns and recurring cadences. Tasks have revision tracking with credit charges after free revisions. Task creator/requester name (`assignedByName`) is enriched server-side on both `GET /api/tasks` and `GET /api/tasks/:id` and displayed on task cards and in the task detail sidebar. Tasks support per-company categories/folders (`taskCategories` table) for organization — managed from the company dashboard tasks tab via CRUD dialog, selectable in task creation and task detail panel, shown as badges on task cards. Tasks also carry `targetMonth` (YYYY-MM, editable in the task detail panel), a `checklist` (jsonb array), `isInternal` flag, and `resourceLinkId`. The task detail panel includes a typed link adder (external URL / campaign / content calendar item) that deep-links to the relevant pages. The retainer scheduler generates recurring tasks 90 days ahead by frequency (daily/weekly/biweekly/monthly/quarterly/annual), setting `targetMonth` and the deliverable credit cost per occurrence, with per-occurrence billing periods and credit reservations; generation also fires when a retainer assignment is activated.
